...
import de.org.mchahn.baselib.io.NulOutputStream;
import de.org.mchahn.baselib.util.MiscUtils;
import de.org.mchahn.baselib.util.NLS;
import de.org.mchahn.baselib.util.Prp;
import de.org.mchahn.baselib.util.Routine;
import de.org.mchahn.baselib.util.VarBool;
import de.org.mchahn.baselib.util.VarDouble;
import de.org.mchahn.baselib.util.VarInt;
import de.org.mchahn.baselib.util.VarLong;
import de.org.mchahn.baselib.util.VarRef;
import de.org.mchahn.baselib.test.util.FileNameMaker;
import de.org.mchahn.baselib.test.util.FilePathWalker;
import de.org.mchahn.baselib.test.util.TestUtils;
import de.org.mchahn.app.trupax.lib.prg.Prg.Concern;
import de.org.mchahn.app.trupax.lib.prg.Prg.Result;
import de.org.mchahn.app.trupax.test.util.Verifier;
import de.org.mchahn.app.trupax.test.util.Verifier.Creator;
...
public class PrgImplTest {
    ...
    public void testMakeDirsAndFiles() throws Exception {
        ...
        final VarRef<Routine.Arg1E<Boolean, File>> fcounter = new VarRef<>();
    ...
    public void test0() throws Exception {
        ...
        final int FILE_COUNT = de.org.mchahn.baselib.test.Control.quick() ? 50 : 5000;
        ...
        prg.registerObjects(new Prg.RegisterObjectsCallbacks() {
            public Result onDirectory(String dir) {
                dcount.v++;
                return Result.ok();
            }
            public void onConfigLocked() {
            }
        });
        ...
        if (UDFTestTool.available()) {
            assertTrue(UDFTestTool.exec(decfl, PrgImpl.BLOCK_SIZE, true, false, false, null));
        }
        assertTrue(TestUtils.removeDir(base, true));
    }
    ...
    public void testLargeVolumes() throws Exception {
        final int LOOPS = 2;
        final long MAX_BYTES = (de.org.mchahn.baselib.test.Control.quick() ? 8 : 6000) * 1000 * 1000L;
        ...
        for (int i = 0; i < LOOPS; i++) {
            ...
            if (UDFTestTool.available()) {
                assertTrue(UDFTestTool.exec(dec, PrgImpl.BLOCK_SIZE, true, false, false, null));
            }
            assertTrue(TestUtils.removeDir(base, true));
    ...
    public void testManyFiles() throws Exception {
        final int LOOPS = 2;
        final long MAX_BYTES = (de.org.mchahn.baselib.test.Control.quick() ? 2 : 200) * 1000 * 1000L;
        ...
        for (int i = 0; i < LOOPS; i++) {
            ...
            Result res = prg.registerObjects(new Prg.RegisterObjectsCallbacks() {
                public Result onDirectory(String dir) {
                    return Result.ok();
                }
                public void onConfigLocked() {
            ...
            System.out.println("verifying...");
            if (UDFTestTool.available()) {
                assertTrue(UDFTestTool.exec(dec, PrgImpl.BLOCK_SIZE, true, false, true, null));
            }
    ...
    public void testFreeSpace() throws Exception {
        ...
        for (final long freeBlocks : new long[] { 0, 1, 100, 4321 }) {
            ...
            if (UDFTestTool.available()) {
                ...
                final VarInt unexpected = new VarInt();
                assertTrue(UDFTestTool.exec(dec, PrgImpl.BLOCK_SIZE, true, true, true, ln -> {
                    ...
                    if (ln.contains("Hard and Soft Write-Protect")) {
                        unexpected.v++;
                    }
                    return true;
                }));
                assertTrue(0 < found.v);
                assertTrue(0 == unexpected.v);
            }
            assertTrue(TestUtils.removeDir(base, true));
            assertTrue(vol.delete());
            delPrg();
    ...
    @Test
    public void testWriteProtect() throws Exception {
        ...
        final String PASSW = "testWriteProtect";
        ...
        final Verifier.Setup vsetup = (new Verifier.Setup() {
            ...
            public int      maxSubDirsPerDir () { return 3; }
            public long     minFileSize      () { return 0; }
            public long     maxFileSize      () { return 45678; }
            public int      minFilesPerDir   () { return 0; }
            public int      maxFilesPerDir   () { return 10; }
            public int      minFileNameLen   () { return 3; }
            public int      maxFileNameLen   () { return 17; }
            public int      maxPathLen       () { return 71; }
            public long     maxBytes         () { return 1000001; }
            public String[] basePath         () { return new String[] { "_wrtprot"  }; }
        });
        ...
        assertTrue(prg.make(PASSW.toCharArray(), new Prg.MakeCallback() {
            public void onFile(String fileName, long fileSize) { }
            public Result onVolumeWrite(long pos) { return Result.ok(); }
            public Result onFreeSpace() { return Result.ok(); }
            public Result onKeyGen() { return Result.ok(); }
        }).isSuccess());

        File dec = new File(base, "testWriteProtect.dec.dump");
        assertTrue(!dec.exists() || dec.delete());
        ...
        if (UDFTestTool.available()) {
            final VarInt found = new VarInt();
            assertTrue(UDFTestTool.exec(dec, PrgImpl.BLOCK_SIZE, true, true, true, ln -> {
                if (ln.contains("#03  Hard and Soft Write-Protect")) {
                    found.v++;
                }
                return true;
            }));
            assertEquals(2, found.v);
        }
        assertTrue(TestUtils.removeDir(base, true));
        assertTrue(vol.delete());
        delPrg();
    ...
    public void testWiping() throws Exception {
        ...
        final int FILE_COUNT = de.org.mchahn.baselib.test.Control.quick() ? 30 : 3000;
        ...
        final VarRef<String> vanishedFile = new VarRef<>();
