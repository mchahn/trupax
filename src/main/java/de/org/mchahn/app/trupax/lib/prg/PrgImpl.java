...
import de.org.mchahn.baselib.io.BlockDeviceWriter;
import de.org.mchahn.baselib.io.DefaultFileSystemFilter;
import de.org.mchahn.baselib.io.FileNode;
import de.org.mchahn.baselib.io.FileRegistrar;
import de.org.mchahn.baselib.io.FileSystem;
import de.org.mchahn.baselib.io.IOUtils;
import de.org.mchahn.baselib.io.LocalFileSystem;
import de.org.mchahn.baselib.io.BlockDeviceImpl.FileBlockDevice;
import de.org.mchahn.baselib.io.FileRegistrar.Callback.Merge;
import de.org.mchahn.baselib.util.BaseLibException;
import de.org.mchahn.baselib.util.CmdLnParser;
import de.org.mchahn.baselib.util.Log;
import de.org.mchahn.baselib.util.MiscUtils;
import de.org.mchahn.baselib.util.Prp;
import de.org.mchahn.baselib.util.Routine;
import de.org.mchahn.baselib.util.VarInt;
import de.org.mchahn.baselib.util.VarRef;
import de.org.mchahn.baselib.util.Prp.Item;

import de.org.mchahn.udflib.UDFReader;
import de.org.mchahn.udflib.UDFWriter;
...
import de.org.mchahn.app.trupax.lib.FATReader;
import de.org.mchahn.app.trupax.lib.NLS;
import de.org.mchahn.app.trupax.lib.Wipe;
import de.org.mchahn.tclib.TCBlockDevice;
import de.org.mchahn.tclib.TCInvalidate;
import de.org.mchahn.tclib.TCReader;

public class PrgImpl extends Prg {
    public static final int BLOCK_SIZE = 512;

    static final String LOG_CTX = "prg";
    ...
    FileNode[]              viewables;
    FileRegistrar           freg;
    BlockDeviceWriter       wrt;
    RegisterObjectsCallbacks rocb;
        ...
        this.freg = new FileRegistrar.InMemory(new Comparator<String>() {
            ...
            void checkConfig() {
                if (null == this.allowMerge) {
                    ...
                    PrgImpl.this.rocb.onConfigLocked();
                }
            }
            Boolean allowMerge;
            Boolean caseMerge;
    ...
    public Result registerObjects(final RegisterObjectsCallbacks cb) {
        dirty();
        this.rocb = cb;
        ...
        final List<FileNode> singleFile = new ArrayList<>();

        final Routine.Arg2<FileRegistrar.Callback.Merge, FileNode, FileNode> onMerge = (arg1, arg2) -> {
        ...
        for (String obj : this.objs) {
            FileNode fn;
            final VarRef<FileSystem.Filter> fsf = new VarRef<>();
            ...
            try {
                if (fn.hasAttributes(FileNode.ATTR_DIRECTORY)) {
                    final VarRef<String> details = new VarRef<>();
                    ...
                    if (!this.freg.add(
                            ...
                            storeFullPath ? null : fnl,
                            null,
                            (nd0, nd1) -> onMerge.call(nd0[0], nd1))) {
    ...
    void loadViewables() {
        if (null == this.viewables) {
            ...
            FileRegistrar.walk(this.freg.root(), fn -> {
                        count.v++;
                        return true;
                    },
                    true,
            ...
            final VarInt i = new VarInt();
            FileRegistrar.walk(this.freg.root(), fn -> {
                        PrgImpl.this.viewables[i.v++] = fn[0];
                        return true;
                    },
                    true,
                    true);
    ...
    public void registerViewSort(final RegObj.Sort sort, final boolean ascending) {
        loadViewables();
        Arrays.sort(this.viewables, (fn1, fn2) -> {
            int r = 0;
            switch (sort) {
                case NAME:
        ...
        try {
            ...
            try {
                ...
                OutputStream os2 = new OutputStream() {
                    ...
                    @Override public void write(byte[] buf, int ofs, int len) throws IOException { t(); os.write(buf, ofs, len); }
                    @Override public void write(byte[] buf)                   throws IOException { t(); os.write(buf); }
                    @Override public void write(int b)                        throws IOException { t(); os.write(b); }
            ...
            BlockDeviceImpl.HookBlockDevice(outdev) {
                ...
                protected boolean onWrite(long num) {
                    return cb.onVolumeWrite(num * BLOCK_SIZE).isSuccess();
                }
            };

            Result result = cb.onKeyGen();
            if (Result.Code.ABORTED == result.code) {
                return result;
            }
            ...
            try {
                ...
                boolean forVeraCrypt = new PrgProps.ForVeraCrypt().get(this.props);
    ...
    public Result resolve() {
        ...
        try {
            this.volumeSize = this.wrt.resolve(new BlockDeviceWriter.Layout() {
                ...
                public boolean writeProtect() {
                    return new PrgProps.WriteProtect().get(PrgImpl.this.props);
                }
    ...
    public Result extract(char[] password, String dir, final ExtractCallback cb) {
        ...
        try {
            ...
            final Key key = new Password(password, null);
            tcr = new TCReader(bdev, key, false, true);
        }
        catch (IOException ioe) {
        ...
        BlockDeviceReader rmul = new BlockDeviceReader.Multiple(new BlockDeviceReader[] {
        ...
        try {
            ...
            rmul.extract(new File(dir), new BlockDeviceReader.Progress2() {
        ...
        catch (BlockDeviceReader.Exception re) {
            switch(re.code) {
